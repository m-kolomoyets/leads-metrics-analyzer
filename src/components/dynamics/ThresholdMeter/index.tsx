    ...
    return (
        <div className={cn('flex flex-col', className)}>
            ...
            <div className="relative h-5 text-xs tabular-nums">
                {value !== null && (
                    <span
                        aria-hidden={true}
                        className={cn(
                            'bg-foreground text-background absolute rounded-sm px-1.5 py-px font-medium',
                            shiftOf(scale.value)
                        )}
            ...
            <div className="relative h-2 w-full rounded-full" style={{ background: gradient }}>
                ...
                {/* The mark and the chip above it are one object: the chip names the figure, the mark
            ...
            <div className="text-muted-foreground relative h-4 pt-0.5 text-xs tabular-nums">
                <span
                    aria-hidden={true}
                    className={cn('text-zone-green absolute', shiftOf(scale.green))}
                    style={{ left: `${scale.green}%` }}
                >
                    {cost(greenBelow)}
                </span>
                <span
                    aria-hidden={true}
                    className={cn('text-zone-red absolute', shiftOf(scale.red))}
                    style={{ left: `${scale.red}%` }}
                >
                    {cost(redAbove)}
                </span>
