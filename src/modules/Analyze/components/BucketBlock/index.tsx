...
// One action bucket (СТОП / ТРИМАЄМО / БУСТ) under an Account: a panel hairlined in its zone, with
// the bucket chip + campaign count and a one-click copy of every campaign id in it.
function BucketBlock({ zone, campaigns, locale, copiedKey, onCopy, copyKey }: BucketBlockProps) {
    ...
    return (
        <div className={cn('flex min-w-48 flex-1 flex-col gap-3 rounded-md border p-3', ZONE_CARD_CLASS[zone])}>
            <StatusChip
                zone={zone}
