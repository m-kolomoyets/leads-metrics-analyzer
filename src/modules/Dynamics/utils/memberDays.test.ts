...
import { dimensionMemberDays, memberDays } from './memberDays';

const september = monthDays('2026-09-15');

const day = (over: Partial<DynamicsHistoryDay> & { reportDate: string }): DynamicsHistoryDay => {
    return { profit: 100, spendPlus: 100, ...over };
};

const stateOn = (days: ReturnType<typeof memberDays>, date: string) => {
...
describe('memberDays', () => {
    it('builds the whole calendar month, not only the days that carry a push', () => {
        const days = memberDays(september, [day({ reportDate: '2026-09-02' })], '2026-09-15');

        expect(days).toHaveLength(30);
    });

    it('separates a day nobody reported from a day still to come', () => {
        const days = memberDays(september, [], '2026-09-15');
    ...
    it('refuses to grade a day with no Spend⁺ — nothing was risked', () => {
        const days = memberDays(september, [day({ reportDate: '2026-09-02', profit: 40, spendPlus: 0 })], '2026-09-15');
    ...
    it('refuses to grade a push that froze no rollup, and says so as a reported day', () => {
        const days = memberDays(
            september,
            [day({ reportDate: '2026-09-02', profit: null, spendPlus: 0 })],
            '2026-09-15'
        );
...
describe('dimensionMemberDays', () => {
    it('says only whether a day was reported, and carries no figure', () => {
        const days = dimensionMemberDays(september, ['2026-09-02'], '2026-09-15');

        expect(stateOn(days, '2026-09-02')).toBe('ungraded');
        expect(stateOn(days, '2026-09-03')).toBe('unreported');
        expect(stateOn(days, '2026-09-20')).toBe('future');
        expect(days[1]).toEqual({ date: '2026-09-02', state: 'ungraded', profit: null, spendPlus: 0, roi: null });
    });
});
