...
function valuesBuilder(first, parameters, types2, columns, options) {
    return first
        .map(
            (row) =>
                '(' +
                columns.map((column) => stringifyValue('values', row[column], parameters, types2, options)).join(',') +
                ')'
        )
        .join(',');
}
...
function Connection(options, queues = {}, { onopen = noop$1, onend = noop$1, onclose = noop$1 } = {}) {
    ...
    function end() {
        return (
            ...
            !connection2.reserved && !initial && !query && sent.length === 0
                ? (terminate(),
                  new Promise((r) => (socket && socket.readyState !== 'closed' ? socket.once('close', r) : r())))
    ...
    function DataRow(x) {
        ...
        for (let i = 0; i < query.statement.columns.length; i++) {
            ...
            query.isRaw
                ? (row[i] =
                      query.isRaw === true ? value : transform.value.from ? transform.value.from(value, column) : value)
    ...
    async function AuthenticationMD5Password(x) {
        const payload =
            'md5' + (await md5(Buffer.concat([Buffer.from(await md5((await Pass()) + user)), x.subarray(9)])));
        write(b().p().str(payload).z(1).end());
    }
    async function SASL() {
...
function tuples(x, columns, xi, transform) {
    ...
    for (let i = 0; i < columns.length; i++) {
        ...
        transform.raw
            ? (row[i] =
                  transform.raw === true ? value : transform.value.from ? transform.value.from(value, column) : value)
            : (row[column.name] = transform.value.from ? transform.value.from(value, column) : value);
    }
    ...
    async function begin(options2, fn) {
        ...
        async function scope(c, fn2, name) {
            ...
            function savepoint(name2, fn3) {
                if (name2 && Array.isArray(name2.raw)) return savepoint((sql3) => sql3.apply(sql3, arguments));
...
function parseOptions(a, b2) {
    ...
    'timeout' in o &&
        (console.log('The timeout option is deprecated, use idle_timeout instead'), (o.idle_timeout = o.timeout));
