        ...
        if (node.wildcard)
            for (let i = node.wildcard.length - 1; i >= 0; i--) {
                ...
                if (prefix) {
                    ...
                    if (!(segment.caseSensitive ? part : (lowerPart ??= part.toLowerCase())).startsWith(prefix))
                        continue;
                }
                if (suffix) {
                    if (isBeyondPath) continue;
...
const loadRouteMatch = async (inner, matchPromises, index) => {
    ...
    if (shouldSkipLoader(inner, matchId)) {
        ...
        const previousRouteMatchId =
            ((activeIdAtIndex && inner.router.stores.matchStores.get(activeIdAtIndex)) || null)?.routeId === routeId
                ? activeIdAtIndex
...
async function loadMatches(arg) {
    ...
    const boundaryIndex =
        beforeLoadNotFound && !inner.preload ? getNotFoundBoundaryIndex(inner, beforeLoadNotFound) : void 0;
...
var RouterCore = class {
    ...
    constructor(options, getStoreConfig) {
        ...
        this.update = (newOptions) => {
            ...
            if (basepathWasUnset || prevBasepath !== nextBasepath || prevRewriteOption !== nextRewriteOption) {
                ...
                this.rewrite =
                    rewrites.length === 0 ? void 0 : rewrites.length === 1 ? rewrites[0] : composeRewrites(rewrites);
                if (this.history) this.updateLatestLocation();
    ...
    matchRoutesLightweight(location) {
        ...
        const canReuseParams =
            lastStateMatch && lastStateMatch.routeId === lastRoute.id && lastStateMatch.pathname === location.pathname;
        let params;
